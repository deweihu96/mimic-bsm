...
def save_metrics(metrics_hist_all, model_dir):
    with open(model_dir + "/metrics.json", 'w') as metrics_file:
        ...
        data.update({"%s_tr" % (name):val for (name,val) in metrics_hist_all[3].items()})
        data.update({"%s_tr" % (name):val for (name,val) in metrics_hist_all[4].items()})
        data.update({"%s_tr" % (name):val for (name,val) in metrics_hist_all[5].items()})
        # pdb.set_trace()
    ...
    if not evaluate:
		...
		#save state dict
                if args.model == 'mtm':
                    ...
                    sd_m = masker.cpu().state_dict()
                    torch.save(sd_m, model_dir + "/masker_model_best_%s.pth" % criterion)
                    if args.gpu:
                        sd_m.cuda()

                    sd_c = classifier.cpu().state_dict()
                    torch.save(sd_c, model_dir + "/classifier_model_best_%s.pth" % criterion)
                    if args.gpu:
                        sd_c.cuda()                    
                else:
